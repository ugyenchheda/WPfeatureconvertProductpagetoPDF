...
class DB_Storage implements Storage {
	...
	public function __construct( $wishlist_id, $user_id ) {
		...
		$this->cache_name  = 'basel_wishlist_' . $this->wishlist_id;
	}
		...
		if ( empty( $cache ) || $cache['expires'] < time() ) {
			...
			$products = $wpdb->get_results(
				$wpdb->prepare(
					"
						SELECT *
						FROM $wpdb->basel_products_table
						WHERE wishlist_id = %d
					",
					$this->wishlist_id
				),
	...
	public function is_table_exists() {
		global $wpdb;
		$products_table_count = $wpdb->query( "SHOW TABLES LIKE '{$wpdb->basel_products_table}%'" );//phpcs:ignore
